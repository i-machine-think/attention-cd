...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, batch_first=False, **kwargs):
        ...
        # print(f'Attention iputs: query: {query.size()}, key: {key.size()}, value: {value.size()}')
        ...
        # print(f'Attention new q: {q.size()}, k: {k.size()}, v: {v.size()}')
        ...
        # print(f'Attention mix: {mix.size()}, focus: {focus.size()}')
...
class PyTorchAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        # print(f'MHA q: {q.size()} k: {k.size()} v: {v.size()}')
...
class SHARNN(nn.Module):
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False):
        ...
        self.num_max_positions = 175 # 2500 # 5000 # 4096 # 2048 # 4096 + 1024 # 2048 # 5000 # 4096 # 1024 # 4096 # 512 # 1024 # 4096 # 4608 # 7168 # 8192 # 6144 # 4608 # 5000 # 4096 # 3072 # 8192 # 4096
        self.num_heads = 1 # 4
        num_layers = nlayers
        self.causal = True
        self.drop = nn.Dropout(dropout)
